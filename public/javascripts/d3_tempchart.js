...
const svg = d3.select("#tempviz").append("svg")
...
svg.append("rect")
    ...
    .on("mousemove", mousemove_temp);
function mousemove_temp() {
            ...
            //console.log(x0,i)
            focus.attr("transform", "translate(" + xScale(timeConv(d.measurement_time)) + "," + yScale(d['temp_f']) + ")");
            focus.select(".tooltip-date").text(timeConv(d.measurement_time).toLocaleString());
            focus.select(".tooltip-temp").text(Math.round(d['temp_f'])+ 'F');
            focus.select(".tooltip-humidity").text(Math.round(d['humidity'])+ '%')
          }
